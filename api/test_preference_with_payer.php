<?php
/**
 * Script de prueba para verificar la creación de preferencia con información del pagador
 */
...
try {
    ...
    echo "Creando preferencia con información del pagador...\n";
    echo "Request: " . json_encode($request, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . "\n\n";
    
    $preference = $client->create($request);
    
    echo "✓ Preferencia creada exitosamente!\n";
    echo "Preference ID: " . $preference->id . "\n";
    echo "Init Point: " . ($preference->init_point ?? 'N/A') . "\n";
    echo "Sandbox Init Point: " . ($preference->sandbox_init_point ?? 'N/A') . "\n";
    
    // Verificar propiedades del objeto
    echo "\nPropiedades de la preferencia:\n";
    if (isset($preference->payer)) {
        echo "Payer: " . json_encode($preference->payer, JSON_PRETTY_PRINT) . "\n";
    }
